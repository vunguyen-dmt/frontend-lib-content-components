...
const thunkActionsKeys = (0, _keyStore.default)(thunkActions);
...
const mockAllowTranscriptImport = {
  data: {
    command: 'import'
  }
};
  ...
  describe('parseVideoShareSetting', () => {
    describe('has no course setting or block setting for video sharing', () => {
      it('should return an object with level equal to block and value equal to null', () => {
        ...
        expect(videoSharingSetting).toEqual({
          level: 'block',
          value: null
        });
      });
...
describe('uploadVideo', () => {
  ...
  fileData.append('file', new File(['content1'], 'file1.mp4', {
    type: 'video/mp4'
  }));
  ...
  it('should call fetch with correct arguments for each file', async () => {
    ...
    response.files.forEach((_ref, index) => {
      let {
